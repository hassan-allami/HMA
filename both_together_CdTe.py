...
wp = 20.7e-3  # hbar wp in eV (w_LO is used)
...
fig.suptitle('CdTe$_{0.99}$O$_{0.01}$, $n = 10^{18}$ cm$^{-3}$, $E_d = 0.38$ eV, $V = 2.2$ eV, '
             '$\Gamma = 10$ meV, $\hbar\omega_p = 20.7$ meV', fontsize=18)

# open files
f_rho = open('Data/rhoj_CdTeO_' + str(x0) + '_Gamma10_n018.txt')  # open file for rhoj
f_D = open('Data/Dj_CdTeO_' + str(x0) + '_Gamma10_n018(new).txt')  # open file for Dj
# read data for rhoj
rho_data = f_rho.read()
